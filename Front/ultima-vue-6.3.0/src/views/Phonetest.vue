<template>
  <div class="card">
      <!-- Add User Button -->
      <button @click="openAddPopup" style="margin-bottom: 20px;">Add User</button>
      ...
      <div v-if="isEditing" class="popup">
          <div class="popup-content">
              <h3>Edit User</h3>
              <input v-model="editUserData.name" placeholder="Name" />
      ...
      <div v-if="isAdding" class="popup">
          <div class="popup-content">
              ...
              <input v-model="newUserData.email" placeholder="Email" />
              <input v-model="newUserData.phonenumber" placeholder="Phone Number" />
...
const fetchUsers = async () => {
  try {
      ...
      users.value = response.data.map(u => ({
          code: u.code || u.U_CODE,
          name: u.name || u.U_NAME,
...
const saveEditedUser = async () => {
  try {
      await api.put(`/api/user/${editUserData.value.code}`, {
...
const saveNewUser = async () => {
  try {
      ...
      users.value.push({
          code: response.data.code || response.data.U_CODE,
